...
#current_data = current_sentence_filter
    ...
    merge_data.append(current_data)
...
'''
wordvec_model = Word2Vec(merge_data, min_count=min_word_frequency_word2vec, size=embed_size_word2vec, window=context_window_word2vec)
...
input1 = Input(shape=(max_sentence_len1+max_sentence_len2, embed_size_word2vec), dtype='float32')
...
for ep in range(epoch_len) :
    print(str(ep+1) + "\n")
    hist = model.fit(doc_X_train, y_train, batch_size=batch_size, epochs=1, verbose=2)
