...
if input_file_num >= 2:
    all_data1, all_exception2, all_path2, all_method2 = pr.open_data(java_auto_exception_json[1], label_name)
...
if input_file_num >= 1:
    ...
    length_exception = [[] for i in range(400)]
    ...
    for i, item in enumerate(standard_data):
        ...
        length_exception[sentence_len-1].append(all_exception[i])
        length_path[sentence_len-1].append(all_path[i])
        length_method[sentence_len-1].append(all_method[i])

    for i in range(400):
        zero, one = ev.return_the_number_of_label_data(length_exception[i])
        limit_num = min(zero, one)
        length_data[i], length_exception[i], length_path[i], length_method[i] = pr.balance_out_data(length_data[i], length_exception[i], length_path[i], length_method[i], limit_num)
        length_data_path = "input_data/real_last/sentence_len/data1/"+input_file+"-code_sentence_len"+str(i+1)+"_frequency"+str(min_word_frequency_word2vec)+".json"
        pr.make_input_data_json(length_data_path, label_name, length_data[i], length_exception[i], length_path[i], length_method[i])
...
if input_file_num >= 2:
    ...
    for i, item in enumerate(standard_data):
        ...
        length_exception[sentence_len-1].append(all_exception2[i])
    ...
    for i in range(400):
        ...
        limit_num = min(zero, one)
        length_data[i], length_exception[i], length_path[i], length_method[i] = pr.balance_out_data(length_data[i], length_exception[i], length_path[i], length_method[i], limit_num)
        length_data_path = "input_data/real_last/sentence_len/data1/"+input_file+"-AST_sentence_len"+str(i+1)+"_frequency"+str(min_word_frequency_word2vec)+".json"
        pr.make_input_data_json(length_data_path, label_name, length_data[i], length_exception[i], length_path[i], length_method[i])
...
if input_file_num >= 3:
    ...
    for i, item in enumerate(standard_data):
        ...
        length_exception[sentence_len-1].append(all_exception3[i])
    ...
    for i in range(400):
        ...
        limit_num = min(zero, one)
        length_data[i], length_exception[i], length_path[i], length_method[i] = pr.balance_out_data(length_data[i], length_exception[i], length_path[i], length_method[i], limit_num)
        length_data_path = "input_data/real_last/sentence_len/data1/"+input_file+"-CAST_sentence_len"+str(i+1)+"_frequency"+str(min_word_frequency_word2vec)+".json"
        pr.make_input_data_json(length_data_path, label_name, length_data[i], length_exception[i], length_path[i], length_method[i])
...
if input_file_num >= 4:
    ...
    for i, item in enumerate(standard_data):
        ...
        length_exception[sentence_len-1].append(all_exception4[i])
    ...
    for i in range(400):
        ...
        limit_num = min(zero, one)
        length_data[i], length_exception[i], length_path[i], length_method[i] = pr.balance_out_data(length_data[i], length_exception[i], length_path[i], length_method[i], limit_num)
        length_data_path = "input_data/real_last/sentence_len/data1/"+input_file+"-depth_num_sentence_len"+str(i+1)+"_frequency"+str(min_word_frequency_word2vec)+".json"
        pr.make_input_data_json(length_data_path, label_name, length_data[i], length_exception[i], length_path[i], length_method[i])
