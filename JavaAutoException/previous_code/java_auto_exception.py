...
input_filename = 'guava-function_AST_partial_code_delete100%'
java_auto_exception_json = '../MakeJSON/output/Exception/'+input_filename+'.json'

#Wordvec_model_name : filename + min_sentence_length + count
#ROC_name : TP,FP,TN,FN + filename + min_sentence_length + epoch + count
#Code_snippet_name : JAL + filename + min_sentence_length + epoch + count
#Model_name : JAL + filename + min_sentence_length + epoch + count + model
...
if data_size_check == False:
    wordvec_model.save("Wordvec_Model/"+str(test_rate)+"/"+filename+"_min"+str(min_sentence_len)+"_"+str(count)+".model")
...
for j, item in enumerate(test_data):
	...
	if len(current_test_filter)>=min_sentence_len:
		...
		final_test_exception.append(test_exception[j])
		final_test_path.append(test_path[j])
		final_test_method.append(test_method[j])
...
if data_size_check == True:
	...
	print(java_auto_exception_json)
...
Y_train = np.empty(shape=[len(updated_train_exception),1], dtype='int32')
...
for j, curr_row in enumerate(updated_train_data):
	...
	Y_train[j,0] = updated_train_exception[j]
...
#TP, FP, TN, FN file save.
	TP_file_name = "Code_snippet/"+str(test_rate)+"/TP_"+filename+"_min" + str(min_sentence_len) + "_ep" + str(ep+1) +"_"+str(count)+".txt"
	FP_file_name = "Code_snippet/"+str(test_rate)+"/FP_"+filename+"_min" + str(min_sentence_len) + "_ep" + str(ep+1) +"_"+str(count)+ ".txt"
	TN_file_name = "Code_snippet/"+str(test_rate)+"/TN_"+filename+"_min" + str(min_sentence_len) + "_ep" + str(ep+1) +"_"+str(count)+ ".txt"
	FN_file_name = "Code_snippet/"+str(test_rate)+"/FN_"+filename+"_min" + str(min_sentence_len) + "_ep" + str(ep+1) +"_"+str(count)+ ".txt"
...
#'''
	...
	roc_name = "ROC/"+str(test_rate) +"/JAL_"+filename+"_ep"+str(ep+1)+"_min"+str(min_sentence_len)+"_"+str(count)
	plt.savefig(roc_name)
	roc_count += 1

	model_json = model.to_json()
	model_name = "Model/"+str(test_rate) + "/JAL_"+filename+"_ep"+str(ep+1)+"_min"+str(min_sentence_len)+"_"+str(count)+"_model.json"
	weight_name = "Model/"+str(test_rate) + "/JAL_"+filename+"_ep"+str(ep+1)+"_min"+str(min_sentence_len)+"_"+str(count)+"_model.h5"
