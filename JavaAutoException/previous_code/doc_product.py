...
input_type1 = 'AST'
input_type2 = 'code'
filename = input_file + '-CAST_d_combine_wordvec_frequency'+str(min_word_frequency_word2vec)+"_LSTM"+str(LSTM_output_size)+"_batch64"
#model_filename = input_file + '-CAST_d_frequency'+str(min_word_frequency_word2vec)
model_filename = 'all_project-code,AST_frequency'+str(min_word_frequency_word2vec)
...
for i, data in enumerate(all_data):
    if all_path[i] != all_path2[i] or all_method[i] != all_method2[i] or all_exception[i] != all_exception2[i]:
        print(i)
        print(all_path[i], all_path2[i])
        print(all_method[i], all_method2[i])
        print(all_exception[i], all_exception2[i])
#sys.exit(1)
...
for ep in range(epoch_len) :
    print(str(ep+1) + "\n")
    hist = model.fit(doc_X_train, y_train, batch_size=batch_size, epochs=1, verbose=1)
