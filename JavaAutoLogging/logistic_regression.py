...
java_auto_logging_json = '../MakeJSON/output/Logging/real_last/'+input_filename+'.json'
filename = input_filename+"_test2"#"_no_semi_frequency"+str(min_word_frequency_word2vec)+"_LSTM"+str(LSTM_output_size)+"_batch64"
...
#current_data = current_sentence_filter
    ...
    for value in current_data:
        if value == ';':
            count += 1
    logistic_data1.append(count)
    logistic_label1.append(item['isLogged'])
    logistic_data2.append(len(current_data))
    logistic_label2.append(item['isLogged'])
...
logistic_data1 = np.asarray(logistic_data1).reshape((-1,1))
logistic_label1 = np.asarray(logistic_label1).reshape((-1,1))
X_train1, X_test1, Y_train1, Y_test1 = train_test_split(logistic_data1,logistic_label1, test_size = 0.1, random_state = 0)
...
pred_y = lr.predict_proba(X_test1)
pred_y = np.argmax(pred_y, axis = 1)
corrects = np.nonzero(pred_y.reshape((-1,1)) == Y_test1)
print("accuracy : ", (len(corrects[0])/len(Y_test1)*100))


logistic_data2 = np.asarray(logistic_data2).reshape((-1,1))
logistic_label2 = np.asarray(logistic_label2).reshape((-1,1))
X_train2, X_test2, Y_train2, Y_test2 = train_test_split(logistic_data2,logistic_label2, test_size = 0.1, random_state = 0)
print(len(X_train2), len(X_test2))
print(len(Y_train2), len(Y_test2))
zero = 0
one = 0
for item in logistic_label2:
    if item == 0 :
        zero = zero + 1
    else :
        one = one +1
...
pred_y = lr.predict_proba(X_test2)
pred_y = np.argmax(pred_y, axis = 1)
corrects = np.nonzero(pred_y.reshape((-1,1)) == Y_test2)
print("accuracy : ", (len(corrects[0])/len(Y_test2)*100))
...
wordvec_model.save("Wordvec_Model/real_last/"+model_filename+"_"+str(count)+".model")
sys.exit(1)
'''
#'''
wordvec_path = "Wordvec_Model/real_last/"+model_filename+"_"+str(model_count)+".model"
...
#if len(current_train_filter)>=min_sentence_len and len(current_train_filter)<=max_sentence_len:
#updated_train_data.append(current_train_filter)
    updated_train_data.append(train_data[j])
...
#   if len(current_test_filter)>=min_sentence_len and len(current_test_filter)<=max_sentence_len:
#    updated_test_data.append(current_test_filter)
    updated_test_data.append(test_data[j])
...
for i,item in enumerate(final_train_data):
    ...
    for value in item:
        if value == ';':
            count += 1
    X_train1.append(count)
    Y_train1.append(final_train_logging[i])
    X_train2.append(len(item))
    Y_train2.append(final_train_logging[i])
for i,item in enumerate(final_test_data):
    count = 0
    for value in item:
        if value == ';':
            count += 1
    X_test1.append(count)
    Y_test1.append(final_test_logging[i])
    X_test2.append(len(item))
    Y_test2.append(final_test_logging[i])

X_train1 = np.asarray(X_train1).reshape((-1,1))
Y_train1 = np.asarray(Y_train1).reshape((-1,1))
X_train2 = np.asarray(X_train2).reshape((-1,1))
Y_train2 = np.asarray(Y_train2).reshape((-1,1))
X_test1 = np.asarray(X_test1).reshape((-1,1))
Y_test1 = np.asarray(Y_test1).reshape((-1,1))
X_test2 = np.asarray(X_test2).reshape((-1,1))
Y_test2 = np.asarray(Y_test2).reshape((-1,1))
...
train_data_json = "input_data/real_last/train/"+str(test_rate)+"/"+filename+".json"
...
test_data_json = "input_data/real_last/test/"+str(test_rate)+"/"+filename+".json"
...
#TP, FP, TN, FN file save.
    ...
    roc_name = "ROC/real_last/"+str(test_rate) +"/JAL_"+filename+"_ep"+str(ep+1)+"_"+str(count)
    plt.savefig(roc_name)
    roc_count += 1

    model_json = model.to_json()
    model_name = "Model/real_last/"+str(test_rate) + "/JAL_"+filename+"_ep"+str(ep+1)+"_"+str(count)+"_model.json"
    weight_name = "Model/real_last/"+str(test_rate) + "/JAL_"+filename+"_ep"+str(ep+1)+"_"+str(count)+"_model.h5"
