...
if input_file_num == 1:
    model_filename = input_filename[0] + "_sentence_balanced_min"+str(st_min_sentence_len)+"_max"+str(st_max_sentence_len)+"_frequency"+str(min_word_frequency_word2vec)
    wordvec_model_name = input_filename[0] + "_frequency"+str(min_word_frequency_word2vec)
else:
    model_filename = input_file + "-CAST_m_"
    for i in range(input_file_num):
        model_filename = model_filename + input_filename[i][len(input_file)+1]
    model_filename = model_filename + "_sentence_balanced_min"+str(st_min_sentence_len)+'_max'+str(st_max_sentence_len)+'_frequency'+str(min_word_frequency_word2vec)
...
filename = model_filename + "_trainNum" + str(train_num)
filename = "stacking_new-"+model_filename


'''
# Check model weight, trainable
max_sentence_len = 100
max_sentence_len2 = 100
max_sentence_len3 = 100
max_sentence_len4 = 100
...
model_h5 = "Model/end2/"+str(test_rate)+"/best/JAL_"+model_filename+"_"+str(count)+"_model.h5"
flag = False
session = K.get_session()
for i, layer in enumerate(model.layers):
    if layer.name == "output":
        model.layers[i-1].trainable = True
        model.layers[i-1].name = model.layers[i-1].name + "_task_layer"
        if hasattr(model.layers[i-1], 'kernel_initializer'):
            model.layers[i-1].kernel.initializer.run(session=session)
        flag = True
    if not flag:
        layer.trainable = False
    else:
        layer.name = layer.name + "_task_layer"
...
if input_file_num >= 1:
    for i in range(st_min_sentence_len, st_max_sentence_len+1):
        java_auto_exception_json = "../JavaAutoException/input_data/real_last/sentence_len/data1/"+input_filename[0]+"_sentence_len"+str(i)+"_frequency"+str(min_word_frequency_word2vec)+".json"
...
if input_file_num >= 2:
    for i in range(st_min_sentence_len, st_max_sentence_len+1):
        java_auto_exception_json = "../JavaAutoException/input_data/real_last/sentence_len/data1/"+input_filename[1]+"_sentence_len"+str(i)+"_frequency"+str(min_word_frequency_word2vec)+".json"
        temp_data, temp_exception, temp_path, temp_method = pr.open_data(java_auto_exception_json, label_name)
        if i == st_min_sentence_len:
...
if input_file_num >= 3:
    for i in range(st_min_sentence_len, st_max_sentence_len+1):
        java_auto_exception_json = "../JavaAutoException/input_data/real_last/sentence_len/data1/"+input_filename[2]+"_sentence_len"+str(i)+"_frequency"+str(min_word_frequency_word2vec)+".json"
        temp_data, temp_exception, temp_path, temp_method = pr.open_data(java_auto_exception_json, label_name)
        if i == st_min_sentence_len:
...
if input_file_num >= 4:
    for i in range(st_min_sentence_len, st_max_sentence_len+1):
        java_auto_exception_json = "../JavaAutoException/input_data/real_last/sentence_len/data1/"+input_filename[3]+"_sentence_len"+str(i)+"_frequency"+str(min_word_frequency_word2vec)+".json"
        temp_data, temp_exception, temp_path, temp_method = pr.open_data(java_auto_exception_json, label_name)
        if i == st_min_sentence_len:
...
# Load model
#model_json = "Model/end2/"+str(test_rate)+"/best/JAL_"+model_filename+"_"+str(model_count)+"_model.json"
model_h5 = "Model/end2/"+str(test_rate)+"/best/JAL_"+model_filename+"_"+str(model_count)+"_model.h5"
...
for i, layer in enumerate(model.layers):
    if layer.name == "output":
        model.layers[i-2].trainable = True
        model.layers[i-2].name = model.layers[i-2].name + "_task_layer"
        model.layers[i-1].trainable = True
        model.layers[i-1].name = model.layers[i-1].name + "_task_layer"
        if hasattr(model.layers[i-1], 'kernel_initializer'):
            model.layers[i-1].kernel.initializer.run(session=session)
        if hasattr(model.layers[i-2], 'kernel_initializer'):
            model.layers[i-2].kernel.initializer.run(session=session)
        flag = True
    if not flag:
        layer.trainable = False
    else:
...
for ep in range(epoch_len) :
    ...
    # Save ROC graph.
    roc_name = "ROC/end2/"+str(test_rate) +filename+"_ep"+str(ep+1)+"_"+str(count)
    ev.save_ROC(roc_count, Y_test_1d, preds, roc_name)
    roc_count += 1

    # Save model.
    model_name = "Model/end2/"+str(test_rate) + filename+"_ep"+str(ep+1)+"_"+str(count)+"_model.json"
    weight_name = "Model/end2/"+str(test_rate) + filename+"_ep"+str(ep+1)+"_"+str(count)+"_model.h5"
