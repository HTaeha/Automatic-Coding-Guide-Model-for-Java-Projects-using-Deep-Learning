...
input_type1 = 'AST'
input_type2 = 'code'
filename = input_file + '-CAST_m'
input_filename1 = input_file + '-' + input_type1
input_filename2 = input_file + '-' + input_type2
java_auto_throw_json = '../MakeJSON/output/Throw/'+input_filename1+'.json'
java_auto_throw_json2 = '../MakeJSON/output/Throw/'+input_filename2+'.json'
wordvec_model_name1 = input_filename1 + '_CAST_m'
wordvec_model_name2 = input_filename2 + '_CAST_m'
epoch_len = 15
...
with open(java_auto_throw_json2,encoding='utf-8-sig') as data_file2:           
...
#current_data = current_sentence_filter
    ...
    all_throw2.append(item['isThrow'])
...
for i in range(len(all_data)):
	if all_throw[i] != all_throw2[i]:
		print(all_method[i], all_method2[i])
	if all_path[i] != all_path2[i] or all_method[i] != all_method2[i]:
		print(all_path[i], all_path2[i])
		print(all_method[i], all_method2[i])
sys.exit(1)
'''
...
wordvec_model.save("Wordvec_Model/"+wordvec_model_name1+"_"+str(count)+".model")
...
wordvec_model2.save("Wordvec_Model/"+wordvec_model_name2+"_"+str(count)+".model")

sys.exit(1)
'''
wordvec_path = "Wordvec_Model/" + wordvec_model_name1 + "_" + str(model_count)+".model"
wordvec_model = Word2Vec.load(wordvec_path)
vocabulary = wordvec_model.wv.vocab

wordvec_path2 = "Wordvec_Model/" + wordvec_model_name2 + "_" + str(model_count)+".model"
...
for old_index in range(len(all_data)):
    ...
    all_throw2[old_index], all_throw2[new_index] = all_throw2[new_index], all_throw2[old_index]
...
if test_rate == 1:
    ...
    train_throw2 = all_throw2[splitLength:]
    test_throw2 = all_throw2[:splitLength-1]
    ...
    train_throw2 = all_throw2[:(test_rate-1)*splitLength-1]+ all_throw2[test_rate*splitLength:]
    test_throw2 = all_throw2[(test_rate-1)*splitLength:test_rate*splitLength-1]
...
for j, item in enumerate(train_data):
    ...
    if (len(current_train_filter)>=min_sentence_len) and (len(current_train_filter2)>=min_sentence_len2):
        ...
        updated_train_throw2.append(train_throw2[j])  
...
for i, item in enumerate(updated_train_throw):
    if item == 0 and count_zero <= limit_zero:
        ...
        final_train_throw2.append(updated_train_throw2[i])
...
for ep in range(epoch_len) :
    print(str(ep+1) + "\n")
    hist = model.fit([X_train, X_train2], y_train, batch_size=batch_size, epochs=1, verbose=2)
