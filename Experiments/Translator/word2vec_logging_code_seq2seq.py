...
first_execution = int(sys.argv[7])
...
if first_execution:
    input_wordvec_model, input_vocabulary = pr.train_word2vec_model(all_data, min_word_frequency_word2vec, embed_size_word2vec, context_window_word2vec, input_model_path)
    target_wordvec_model, target_vocabulary = pr.train_word2vec_model(all_data2, min_word_frequency_word2vec, embed_size_word2vec, context_window_word2vec, target_model_path)
else:
...
max_encoder_seq_length = max([len(txt) for txt in all_data])
max_decoder_seq_length = max([len(txt) for txt in all_data2])
...
encoder_input_data = np.zeros(
    (len(input_texts), max_encoder_seq_length, embed_size_word2vec),
    dtype='float32')
decoder_input_data = np.zeros(
    (len(input_texts), max_decoder_seq_length, embed_size_word2vec),
    dtype='float32')
...
for j, curr_row in enumerate(all_data):
    ...
    for item in curr_row:
        if item in input_vocabulary:
            encoder_input_data[j, sequence_cnt, :] = input_wordvec_model[item] 
            sequence_cnt += 1                
            if sequence_cnt == max_encoder_seq_length-1:
                break                

for j, curr_row in enumerate(all_data2):
    sequence_cnt = 0         
    for item in curr_row:
        if item in target_vocabulary:
            decoder_target_data[j, sequence_cnt, :] = target_wordvec_model[item] 
            sequence_cnt += 1                
            if sequence_cnt == max_decoder_seq_length-1:
                break                
...
encoder_inputs = Input(shape=(None, embed_size_word2vec))
...
decoder_inputs = Input(shape=(None, embed_size_word2vec))
...
decoder_dense = Dense(embed_size_word2vec, activation='softmax')
