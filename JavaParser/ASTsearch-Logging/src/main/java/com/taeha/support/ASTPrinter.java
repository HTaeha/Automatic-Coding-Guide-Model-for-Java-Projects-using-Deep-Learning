...
import com.taeha.ASTsearch_Logging.JAL;
import com.taeha.support.*;

/**
 * @overview This program prints the abstract syntax tree of a given Java
 *           program.
 * 
 * @author ducmle <br>
 ...
 * 
 *         <p>
 *         <b>Extension</b>: print node type and, if node type is non-composite
 *         (i.e. not specified among {@link #CompositeNodeClasses}), then also
 *         print the node value
 * @version
 */
public class ASTPrinter{

	private static final Class[] CompositeNodeClasses = { CompilationUnit.class, ClassOrInterfaceDeclaration.class,
			MethodDeclaration.class, BlockStmt.class, IfStmt.class, WhileStmt.class, ForEachStmt.class, ForStmt.class,
			DoStmt.class, SwitchStmt.class, SwitchEntryStmt.class, TryStmt.class, CatchClause.class, };
	...
	public static void print(File file) {
		...
		String filePath = "D:\\Google 드라이브\\Research\\JavaAutoLogging\\JavaParser\\output\\Sample.txt";
	...
	protected static void printNode(Node n, int indentLevel) {
		...
		try {
			...
			Class c = MethodDeclaration.class;
			if (n.getClass().equals(c)) {
				fileWrite.write(indent("0", indentLevel) + "METHOD");
				fileWrite.newLine();
				fileWrite.flush();
			}
			if (isComposite(n)) {
				// n is a composite node -> print the type name only
				fileWrite.write(indent("  ", indentLevel) + nodeType.getSimpleName());
				fileWrite.newLine();
				fileWrite.flush();
