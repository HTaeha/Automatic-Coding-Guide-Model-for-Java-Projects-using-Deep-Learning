...
input_type1 = 'AST'
input_type2 = 'code'
filename = input_file + '-AdaBoost_CAST_d'
...
wordvec_model_name = input_file + "-CAST_d"
epoch_len = 15
...
sys.exit(1)
'''
wordvec_path = "../JavaAutoException/Wordvec_Model/real_final/"+wordvec_model_name+"_"+str(model_count)+".model"
...
doc_X_train = np.empty(shape=[len(final_train_data), max_sentence_len1+max_sentence_len2, embed_size_word2vec], dtype='float32')
...
for j, curr_row in enumerate(final_train_data2):
    ...
    for item in curr_row:
        if item in vocabulary:
            ...
            doc_X_train[j, max_sentence_len1 + sequence_cnt, :] = wordvec_model[item]
            sequence_cnt = sequence_cnt + 1                
    ...
    for k in range(sequence_cnt, max_sentence_len2):
        ...
        doc_X_train[j, max_sentence_len1+k, :] = np.zeros((1,embed_size_word2vec))

doc_X_test = np.empty(shape=[len(final_test_data), max_sentence_len1+max_sentence_len2, embed_size_word2vec], dtype='float32')
...
for j, curr_row in enumerate(final_test_data2):
    ...
    for item in curr_row:
        if item in vocabulary:
            ...
            doc_X_test[j, max_sentence_len1+sequence_cnt, :] = wordvec_model[item] 
            sequence_cnt = sequence_cnt + 1                
            if sequence_cnt == max_sentence_len2-1:
                break                
    for k in range(sequence_cnt, max_sentence_len2):
        X_test2[j, k, :] = np.zeros((1,embed_size_word2vec))        
        doc_X_test[j, max_sentence_len1+k, :] = np.zeros((1,embed_size_word2vec))        
...
nsamples,nx,ny = doc_X_train.shape
d2_train_dataset = doc_X_train.reshape((nsamples, nx*ny))
nsamples,nx,ny = doc_X_test.shape
d2_test_dataset = doc_X_test.reshape((nsamples, nx*ny))
