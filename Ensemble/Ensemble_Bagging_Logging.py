...
input_type1 = 'AST'
input_type2 = 'code'
filename = input_file + '-ensemble_code,AST'
input_filename1 = input_file + '-' + input_type1
input_filename2 = input_file + '-' + input_type2
model_filename = input_filename1
model_filename2 = input_filename2
java_auto_logging_json = '../MakeJSON/output/Logging/final/'+input_filename1+'.json'
java_auto_logging_json2 = '../MakeJSON/output/Logging/final/'+input_filename2+'.json'
wordvec_model_name1 = input_filename1
wordvec_model_name2 = input_filename2
epoch_len = 15
model_test_rate = int(sys.argv[2])
count = 1
model_count = 1
...
with open(java_auto_logging_json2,encoding='utf-8-sig') as data_file2:           
...
#current_data = current_sentence_filter
    ...
    all_logging2.append(item['isLogged'])
...
wordvec_model.save("Wordvec_Model/Logging/"+wordvec_model_name1+"_"+str(count)+".model")

wordvec_model2 = Word2Vec(all_data2, min_count=min_word_frequency_word2vec, size=embed_size_word2vec, window=context_window_word2vec)
vocabulary2 = wordvec_model2.wv.vocab
wordvec_model2.save("Wordvec_Model/Logging/"+wordvec_model_name2+"_"+str(count)+".model")
sys.exit(1)
'''
wordvec_path = "../JavaAutoLogging/Wordvec_Model/final/"+str(model_test_rate)+"/"+wordvec_model_name1+"_"+str(model_count)+".model"
wordvec_model = Word2Vec.load(wordvec_path)
vocabulary = wordvec_model.wv.vocab

wordvec_path2 = "../JavaAutoLogging/Wordvec_Model/final/"+str(model_test_rate)+"/"+wordvec_model_name2+"_"+str(model_count)+".model"
...
for old_index in range(len(all_data)):
    ...
    all_logging2[old_index], all_logging2[new_index] = all_logging2[new_index], all_logging2[old_index]
...
if model_test_rate == 1:
    ...
    train_logging2 = all_logging2[splitLength:]
    test_logging2 = all_logging2[:splitLength-1]
    ...
    train_logging = all_logging[:(model_test_rate-1)*splitLength-1]+ all_logging[model_test_rate*splitLength:]
    test_logging = all_logging[(model_test_rate-1)*splitLength:model_test_rate*splitLength-1]
    ...
    train_data2 = all_data2[:(model_test_rate-1)*splitLength-1] + all_data2[model_test_rate*splitLength:]
    test_data2 = all_data2[(model_test_rate-1)*splitLength:model_test_rate*splitLength-1]
    train_logging2 = all_logging2[:(model_test_rate-1)*splitLength-1]+ all_logging2[model_test_rate*splitLength:]
    test_logging2 = all_logging2[(model_test_rate-1)*splitLength:model_test_rate*splitLength-1]
    train_path2 = all_path2[:(model_test_rate-1)*splitLength-1]+ all_path2[model_test_rate*splitLength:]
    test_path2 = all_path2[(model_test_rate-1)*splitLength:model_test_rate*splitLength-1]
    train_method2 = all_method2[:(model_test_rate-1)*splitLength-1]+ all_method2[model_test_rate*splitLength:]
    test_method2 = all_method2[(model_test_rate-1)*splitLength:model_test_rate*splitLength-1]
...
for j, item in enumerate(train_data):
    ...
    current_train_filter2 = [word2 for word2 in train_data2[j] if word2 in vocabulary2]
    if (len(current_train_filter)>=min_sentence_len) and (len(current_train_filter2)>=min_sentence_len2):
        ...
        updated_train_logging2.append(train_logging2[j])  
...
for j, item in enumerate(test_data):
    ...
    current_test_filter2 = [word2 for word2 in test_data2[j] if word2 in vocabulary2]  
...
for i, item in enumerate(updated_train_logging):
    if item == 0 and count_zero <= limit_zero:
        ...
        final_train_logging2.append(updated_train_logging2[i])
...
for j, curr_row in enumerate(final_train_data2):
    ...
    for item in curr_row:
        if item in vocabulary2:
            X_train2[j, sequence_cnt, :] = wordvec_model2[item] 
            sequence_cnt = sequence_cnt + 1                
...
output2 = Dense(2, activation='softmax', name='output2')(after_dp_2)
'''
model_json = "../JavaAutoLogging/Model/final/"+str(model_test_rate)+"/JAL_"+model_filename+"_"+str(model_count)+"_model.json"
model_h5 = "../JavaAutoLogging/Model/final/"+str(model_test_rate)+"/JAL_"+model_filename+"_"+str(model_count)+"_model.h5"
...
model_json2 = "../JavaAutoLogging/Model/final/"+str(model_test_rate)+"/JAL_"+model_filename2+"_"+str(model_count)+"_model.json"
model_h5_2 = "../JavaAutoLogging/Model/final/"+str(model_test_rate)+"/JAL_"+model_filename2+"_"+str(model_count)+"_model.h5"
...
for i in range(len(predict)):
    ...
    predictY.append(temp.index(max(temp))%2)
...
TP_file_name = "Code_snippet/Logging/"+str(model_test_rate)+"/TP_"+filename+"_"+str(count)+".txt"
FP_file_name = "Code_snippet/Logging/"+str(model_test_rate)+"/FP_"+filename+"_"+str(count)+ ".txt"
TN_file_name = "Code_snippet/Logging/"+str(model_test_rate)+"/TN_"+filename+"_"+str(count)+ ".txt"
FN_file_name = "Code_snippet/Logging/"+str(model_test_rate)+"/FN_"+filename+"_"+str(count)+ ".txt"
...
roc_name = "ROC/"+str(model_test_rate) +"/JAL_"+filename+"_"+str(count)
plt.savefig(roc_name)
roc_count += 1
'''
'''
model_json = model.to_json()
model_name = "Model/Logging/"+str(model_test_rate) + "/JAL_"+filename+"_"+str(count)+"_model.json"
weight_name = "Model/Logging/"+str(model_test_rate) + "/JAL_"+filename+"_"+str(count)+"_model.h5"
