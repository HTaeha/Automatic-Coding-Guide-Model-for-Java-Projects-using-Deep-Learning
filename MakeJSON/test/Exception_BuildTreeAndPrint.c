...
void printTree_preorder(Tree *T, int isTryCatch, char *str_path, char *str_method, FILE *fp_w, int last);
void printTree(Tree *T, int isTryCatch, char *str_path, char *str_method, FILE *fp_w, int last);
int main(){
	...
	int i;
	char *str_try = "TRY";
	char *str_catch = "CATCH";
	char *str_finally = "FINALLY";
	int numOfsnippets = 0;
	int depth;
	TreeNode *parentNode;
	int count_tab = 0;
...
//	fp_w = fopen("output/Sample/Sample-real.json", "w");
	fp_r = fopen("input/Exception/hbase-exception_expression++.txt","r");
	fp_w = fopen("output/Exception/hbase-exception_expression++.json", "w");
	...
	while(1){
		...
		if(bup[count_tab] <= '9' && bup[count_tab] >= '0'){
			if(bup[1+count_tab] <= '9' && bup[1+count_tab] >= '0'){
				...
				if(strncmp(bup+2+count_tab, str_try, 3) == 0 || strncmp(bup+2+count_tab, str_catch, 5) || strcmp(bup+2+count_tab, str_finally) == 0){
					isTryCatch = 1;
					continue;
				}else{
				...
				if(strncmp(bup+1+count_tab, str_try, 3) == 0 || strncmp(bup+1+count_tab, str_catch, 5) == 0 || strcmp(bup+1+count_tab, str_finally) == 0){
					isTryCatch = 1;
					continue;
				}else{
			...
			if(numOfsnippets != 0){
                if(tree_based){
                    printTree(T, isTryCatch, str_p, str_m, fp_w, 0);
                }else{
                    printTree_preorder(T, isTryCatch, str_p, str_m, fp_w, 0);
                }
				ClearTree(T->root);
				free(T);
				isTryCatch = 0;
    ...
    if(tree_based){
        printTree(T, isTryCatch, str_p, str_m, fp_w, 1);
    }else{
        printTree_preorder(T, isTryCatch, str_p, str_m, fp_w, 1);
    }
