...
int main(){
    ...
    //	fp_r = fopen("input/guava/guava-real.txt","r");
    //	fp_w = fopen("output/guava/guava-real.json", "w");
    fp_r = fopen("input/Exception/real_last/guava-AST.txt","r");
    fp_w = fopen("output/Exception/real_last/guava-depth_num.json", "w");
    ...
    while(1){
        ...
        if(path_flag == 1){
            ...
            sprintf(str, "%d", depth-code_snippet_depth+1);
